...
def get_magn_roi(any_image: ndarray) -> ndarray:
    ...
    re_roi = np.zeros(cfg.dsize_roi, dtype=np.float32)
    im_roi = np.zeros(cfg.dsize_roi, dtype=np.float32)
    magn_roi_norm = np.zeros(cfg.dsize_roi, dtype=np.float32)
...
def get_magn_half(magn_roi: ndarray) -> ndarray:

    size_roi_half_1 = cfg.size_roi_half + 1

    shape_magn_half = (size_roi_half_1, cfg.size_roi)

    magn_half = np.zeros(shape_magn_half, dtype=np.float32)
